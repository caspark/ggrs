...
impl<T: Config> SessionBuilder<T> {
    ...
    pub fn start_p2p_session(
        ...
        let mut addr_to_handles = HashMap::<PlayerType<T::Address>, HashSet<PlayerHandle>>::new();
        ...
        let playing_player_handles: HashSet<PlayerHandle> = self
            .player_reg
            .playing_player_handles()
            .into_iter()
            .collect();
        for (player_type, handles) in addr_to_handles.into_iter() {
            match player_type {
                ...
                PlayerType::Spectator(peer_addr) => {
                    self.player_reg.spectators.insert(
                        peer_addr.clone(),
                        // the host of the spectator pretends all players are local players so that it
                        // sends inputs for all players
                        self.create_endpoint(
                            handles,
                            peer_addr.clone(),
    ...
    fn create_endpoint(
        ...
        peer_addr: T::Address,
        local_players: HashSet<PlayerHandle>,
    ) -> UdpProtocol<T> {
        UdpProtocol::new(
            handles,
            peer_addr,
